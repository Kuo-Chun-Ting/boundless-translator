...
4. 對 `Build/Whisper Translate.app` 執行：
...
本機建置預設使用 Keychain 裡的 `Whisper Translate Local Development`。需要切換簽章身分時，以 `WHISPER_TRANSLATE_SIGNING_IDENTITY` 指定；變數只存憑證名稱或 SHA-1，不得存放或提交私鑰。

## 部署

完成驗證後，必須把本次建立的 App 部署到 `/Applications/Whisper Translate.app`：

1. 使用 `pgrep -x WhisperTranslate` 檢查已安裝的 App 是否仍在執行。
2. 如果正在執行，只終止名稱完全相符的 `WhisperTranslate` 程序，並等待 `pgrep -x WhisperTranslate` 確認程序已停止；不得使用模糊程序名稱終止其他 App。
3. 在唯一的暫存目錄中暫存新版並保留舊版備份，再替換 `/Applications/Whisper Translate.app`。部署失敗時恢復舊版，不得留下部分複製的 App。
