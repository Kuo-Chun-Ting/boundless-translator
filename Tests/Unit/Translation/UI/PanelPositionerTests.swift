...
@Test
func test_origin_when_window_fits_below_pointer_then_offsets_from_pointer() {
    // Arrange
    let positioner = WindowPositioner(pointerOffset: 12)
    let visibleFrame = CGRect(x: 0, y: 0, width: 1_000, height: 800)

    // Act
    let origin = positioner.origin(
        pointer: CGPoint(x: 400, y: 500),
        windowSize: CGSize(width: 300, height: 180),
...
@Test
func test_origin_when_window_exceeds_right_edge_then_clamps_horizontally() {
    // Arrange
    let positioner = WindowPositioner(pointerOffset: 12)
...
@Test
func test_origin_when_window_exceeds_bottom_edge_then_clamps_vertically() {
    // Arrange
    let positioner = WindowPositioner(pointerOffset: 12)
    let visibleFrame = CGRect(x: 100, y: 50, width: 800, height: 600)
...
@Test
func test_origin_when_window_exceeds_top_edge_then_clamps_vertically() {
    // Arrange
    let positioner = WindowPositioner(pointerOffset: 12)
    let visibleFrame = CGRect(x: 100, y: 50, width: 800, height: 600)
...
@Test
func test_resizedOrigin_when_window_grows_after_being_moved_then_preserves_top_left() {
    // Arrange
    let positioner = WindowPositioner(pointerOffset: 12)
    ...
    let origin = positioner.resizedOrigin(
        ...
        newWindowSize: CGSize(width: 560, height: 340),
        visibleFrame: visibleFrame
    )
