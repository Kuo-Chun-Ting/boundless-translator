...
@Test @MainActor
func test_setEnabled_when_serviceRemainsDisabled_then_namesBoundlessTranslatorInGuidance() {
    // Arrange
    let mock_service = LoginItemServiceMock(
        isRegistered: false,
        registrationUpdatesState: false
    )
    let controller = LaunchAtLoginController(service: mock_service)
    ...
    #expect(!controller.isEnabled)
    #expect(
        controller.errorMessage == "Allow Boundless Translator in System Settings > General > Login Items."
    )
}
...
private final class LoginItemServiceMock: LoginItemServicing {
    ...
    private let registrationUpdatesState: Bool

    init(
        isRegistered: Bool,
        registrationError: Error? = nil,
        unregistrationError: Error? = nil,
        registrationUpdatesState: Bool = true
    ) {
        self.isRegistered = isRegistered
        self.registrationError = registrationError
        self.unregistrationError = unregistrationError
        self.registrationUpdatesState = registrationUpdatesState
    }
