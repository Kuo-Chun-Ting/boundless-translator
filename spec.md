...
- Preserve this behavior after opening and closing translation, clearing selection with a blank click, and switching to another app and back.
- A drag beginning in blank image space clears the old selection and begins native text selection when it reaches text, without requiring another click.
...
- The ten Live Text GUI cases use the existing E2E runner and a fixture containing the production screenshot and translation windows. Five cases cover hover cursors and selection; five cover dragging from blank image space into text. Each group covers a fresh screenshot, an existing selection, translation just closed, selection cleared, and returning from another app. Fixture state is exchanged under `Build/LiveTextTestState` and removed after testing. Run these cases with `-only-testing:BoundlessTranslatorE2ETests/LiveTextSelectionGUITests`; the normal E2E script retains its three product journeys.
