...
- Every packaged App enables App Sandbox and outgoing-network access. The test DMG is subscription-free; the App Store build enables subscription access control. Both editions use the same sources and bundle identifier, not a separate Sandbox test edition. A successful build does not prove runtime compatibility or App Review eligibility.
...
- `Scripts/DMG/build_app.sh` builds the Developer ID App through Xcode for the test DMG. The remaining scripts in `Scripts/DMG/` only package, verify and notarize the test DMG; they do not archive, export or upload an App Store build.
