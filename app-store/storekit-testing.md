...
- `Scripts/verify.sh subscription` 仍執行訂閱單元、元件與 Xcode 專案設定測試。跳過整合測試時明確顯示警告；其他測試失敗仍回傳失敗。
- macOS 或 Xcode 版本／build 改變後自動恢復執行。手動重試：`Scripts/TestRunners/run_storekit_tests.sh --force`。
